...
{
  "metadata": {
    "name": "netperf-container"
  },
  "image": {
    "image": "localhost:5000/mon-image-netperf"
  },
  "log_path": "netperf-container.log"
}
EOF
...
run_with_runtime() {
    ...
    case $runtime in
      docker)
        timeout 15s docker run --rm "$IMAGE_NAME"
        ;;
      podman)
        timeout 15s podman run --rm "$IMAGE_NAME"
        ;;
      crio)
         timeout 15s run_crio_once
        ;;
      *)
