...
run_crio_once() {
  ...
  local power_file="$1" # Ce paramètre n'est pas utilisé dans cette fonction mais conservé pour cohérence
  local time_file="$2"
...
EOF
  ...
  # Boucle while true pour attendre l'état "CONTAINER_RUNNING"
  while true; do
    STATUS=$(sudo crictl inspect "$CONTAINER_ID" | grep '"state"' | awk -F'"' '{print $4}')
    if [[ "$STATUS" == "CONTAINER_RUNNING" ]]; then
      break # Sortir de la boucle une fois que le conteneur est en marche
    fi
    sleep 0.001 # Délai minimal pour éviter de surcharger le CPU avec des vérifications trop rapides
  done
  ...
  # sudo crictl logs "$CONTAINER_ID" # Décommenter si vous voulez voir les logs
...
run_with_runtime() {
  ...
  case $runtime in
    docker)
      ...
      # Boucle while true pour attendre l'état "running"
      while true; do
        STATUS=$(docker inspect -f '{{.State.Status}}' "$CONTAINER_ID")
        if [[ "$STATUS" == "running" ]]; then
          break # Sortir de la boucle
        fi
        sleep 0.001 # Délai minimal
      done
      END=$(date +%s%3N)
      DURATION=$((END - START))
      echo "$DURATION" > "$time_file"
